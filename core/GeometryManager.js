...
class DuocylinderGeometry extends BaseGeometry {
    getShaderCode() {
        return `
            // Uniforms used: u_dimension, u_time, u_morphFactor, u_shellWidth, u_dataChannel1, u_dataChannel2, u_dataChannel3, u_rotationSpeed, u_universeModifier
            float calculateLattice(vec3 p) {
                float r1 = 0.6 + u_morphFactor * 0.4; // Main radius of first cylinder part, morphable
                float r2 = 0.3 + u_dataChannel1 * 0.3; // Main radius of second cylinder part, data-driven
                float dynamicShellWidth = max(0.005, u_shellWidth * (1.0 - u_dataChannel2 * 0.7));

                // Base 3D shape (e.g., a simple pulsating sphere as fallback)
                float lattice3D = 0.5 + 0.5 * sin(length(p) * (8.0 + u_dataChannel3 * 5.0) - u_time * u_rotationSpeed);
                ...
                if (dim_factor > 0.01) {
                    ...
                    float w_coord = cos(length(p.xy) * 1.8 - u_time * 0.4 * u_rotationSpeed)
                                  ...
                                  * dim_factor * (0.5 + u_morphFactor * 0.3 + u_dataChannel3 * 0.2);
                    vec4 p4d = vec4(p, w_coord);

                    // Apply 4D rotations (standard rotations from other geometries)
                    float baseSpeed = u_rotationSpeed * 0.9;
                    float time_rot1 = u_time * 0.30 * baseSpeed + u_dataChannel1 * 0.3; // Rotation XW
                    float time_rot2 = u_time * 0.25 * baseSpeed + u_dataChannel2 * 0.35; // Rotation YZ
                    float time_rot3 = u_time * -0.20 * baseSpeed + u_dataChannel3 * 0.4; // Rotation ZW
