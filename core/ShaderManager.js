...
class ShaderManager {
    ...
    _getBaseFragmentShaderSource() {
        return `
            ...
            uniform float u_dataChannel1; uniform float u_dataChannel2; uniform float u_dataChannel3;
            ...
            */
            ...
            void main() {
                ...
                float camRotY = u_time * 0.05 * u_rotationSpeed + u_dataChannel2 * 0.1; float camRotX = sin(u_time * 0.03 * u_rotationSpeed) * 0.15 + u_dataChannel3 * 0.1;
                ...
                color = mix(color, u_secondaryColor, smoothstep(0.2, 0.7, u_dataChannel2) * latticeValue * 0.6);
                if (abs(u_colorShift) > 0.01) { vec3 hsv = rgb2hsv(color); hsv.x = fract(hsv.x + u_colorShift * 0.5 + u_dataChannel3 * 0.1); color = hsv2rgb(hsv); }
                ...
                if (u_glitchIntensity > 0.001) {
                     ...
                     vec3 colorR = mix(u_backgroundColor, u_primaryColor, latticeR); colorR = mix(colorR, u_secondaryColor, smoothstep(0.2, 0.7, u_dataChannel2) * latticeR * 0.6);
                     vec3 colorB = mix(u_backgroundColor, u_primaryColor, latticeB); colorB = mix(colorB, u_secondaryColor, smoothstep(0.2, 0.7, u_dataChannel2) * latticeB * 0.6);
                     if (abs(u_colorShift) > 0.01) { vec3 hsvR=rgb2hsv(colorR); hsvR.x=fract(hsvR.x+u_colorShift*0.5+u_dataChannel3*0.1); colorR=hsv2rgb(hsvR); vec3 hsvB=rgb2hsv(colorB); hsvB.x=fract(hsvB.x+u_colorShift*0.5+u_dataChannel3*0.1); colorB=hsv2rgb(hsvB); }
